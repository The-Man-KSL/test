    ...
    if Config['UnionHeist']['framework']['name'] == 'ESX' then
        ...
        ESX.RegisterServerCallback('unionheist:server:checkTime', function(source, cb, index)
            ...
            if (os.time() - lastRob) < Config['UnionHeist']['nextRob'] and lastRob ~= 0 then
                ...
                TriggerEvent("Scoreboard:SetHeistBusy", "Union heist", 900000)
                cb({status = true})
            end
        end)
        ...
        if player then
            if type == 'money' then
                ...
                if dist > 100.0 then
                    ...
                    if Config['UnionHeist']['black_money']['status'] then
        ...
        if player then
            if type == 'money' then
                ...
                if dist > 30.0 then
                    ...
                    if Config['UnionHeist']['black_money']['status'] then
                        player.Functions.AddItem(Config['UnionHeist']['black_money']['itemName'], count)
        ...
        if player then
            for k, v in pairs(Config['UnionHeist']['rewardItems']) do
                ...
                if playerItem ~= nil and playerItem.amount > 0 then
                    ...
                    if Config['UnionHeist']['black_money']['status'] then
                        player.Functions.AddItem(Config['UnionHeist']['black_money']['itemName'], playerItem.amount * v['sellPrice'])
                    else
...
RegisterCommand('pdunion', function(source, args)
    ...
    if Config['UnionHeist']['framework']['name'] == 'ESX' then
        ...
        if player then
            if player['job']['name'] == Config['UnionHeist']['setjobForPolice'] then
                DeleteEntity(cutter)
                TriggerClientEvent('unionheist:client:sync', -1, 'resetHeist')
            else

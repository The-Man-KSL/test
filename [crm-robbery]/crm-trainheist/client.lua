        ...
        if status then
            ESX.TriggerServerCallback('trainheist:server:checkTime', function(time)
                if time then
                    ...
                    ambushBlip = addBlip(Config['TrainHeist']['setupTrain']['pos'], 57, 1, Strings['ambush_blip'])
                    repeat
                        local ped = PlayerPedId()
                        local pedCo = GetEntityCoords(ped)
...
function FinishHeist()
    ...
    loadModel('baller')
    buyerBlip = addBlip(Config['TrainHeist']['finishHeist']['buyerPos'], 57, 0, Strings['buyer_blip'])

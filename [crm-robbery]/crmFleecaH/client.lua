        ...
        for k, v in pairs(Config['FleecaHeist']) do
            ...
            if dist <= 10.0 then
                ...
                if IsPedShooting(ped) then
                    StartFleecaHeist(k)
                    Wait(5000)
                    TriggerServerEvent('InteractSound_CL:PlayOnSource', 'payday2', Config.Volume)
                    Wait(5000)
                end
            end
        end
...
AddEventHandler('fleecaheist:client:policeAlert', function(targetCoords)
    ...
    TriggerServerEvent('cd_dispatch:AddNotification', {
        ...
        message = 'A '..data.sex..' robbing a Fleeca Bank at '..data.street, 
        flash = 1,

...
window.addEventListener('message', function(event) {
    ...
    if (item.action == 'getMinimap') {
        $.post('https://crm-hud/changeMinimap', JSON.stringify({minimap: localStorage.getItem("minimap")}));
    }
...
document.addEventListener("keyup", (event) => {
    if (event.keyCode == 27) {
        $.post('https://crm-hud/closeCustomizationMenu');
    }
});
        ...
        if ($(this).data('option')) {
            ...
            if ($(this).data('option') == 'minimap') {
                if ($(this).data('name') == 'circle') {
                    ...
                    $.post('https://crm-hud/changeMinimap', JSON.stringify({minimap: 'circle'}));
                } else {
                    ...
                    $.post('https://crm-hud/changeMinimap', JSON.stringify({minimap: 'square'}));
                }
            }
        }

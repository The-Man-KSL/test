...
function client.openInventory(inv, data)
	...
	if inv == 'dumpster' and cache.vehicle then
		return lib.notify({ id = 'inventory_right_access', type = 'error', description = locale('inventory_right_access'), position = 'top' })
	end
	...
	if canOpenInventory() then
		...
		if inv == 'shop' and invOpen == false then
			if cache.vehicle then
				return lib.notify({ id = 'cannot_perform', type = 'error', description = locale('cannot_perform'), position = 'top' })
			end
		...
		if left then
			...
			if invOpen == false then lib.notify({ id = 'inventory_right_access', type = 'error', description = locale('inventory_right_access'), position = 'top' }) end
			if invOpen then client.closeInventory() end
		end
	else lib.notify({ id = 'inventory_player_access', type = 'error', description = locale('inventory_player_access'), position = 'top' }) end
end
	...
	if item and usingItem then
		...
		if success then
			if item.notification then
				lib.notify({ description = item.notification, position = 'top' })
			end
...
local function useSlot(slot)
	...
	if canUseItem(data.ammo and true) then
		if data.component and not currentWeapon then
			return lib.notify({ id = 'weapon_hand_required', type = 'error', description = locale('weapon_hand_required'), position = 'top' })
		end
		...
		if durability and durability <= 100 and consume then
			if durability <= 0 then
				return lib.notify({ type = 'error', description = locale('no_durability', label), position = 'top' })
			elseif consume ~= 0 and consume < 1 and durability < consume * 100 then
				return lib.notify({ type = 'error', description = locale('not_enough_durability', label), position = 'top' })
			end
		end
		...
		elseif data.weapon then
            ...
            if data.hash ~= GetSelectedPedWeapon(playerPed) then
                return lib.notify({ type = 'error', description = locale('cannot_use', data.label), position = 'top' })
            end
			...
			elseif data.component then
				...
				for componentIndex = 1, #weaponComponents do
					if componentType == Items[weaponComponents[componentIndex]].type then
						return lib.notify({ id = 'component_slot_occupied', type = 'error', description = locale('component_slot_occupied', componentType), position = 'top' })
					end
				end
				...
				for i = 1, #components do
					...
					if DoesWeaponTakeWeaponComponent(currentWeapon.hash, component) then
						if HasPedGotWeaponComponent(playerPed, currentWeapon.hash, component) then
							lib.notify({ id = 'component_has', type = 'error', description = locale('component_has', label), position = 'top' })
						else
				...
				lib.notify({ id = 'component_invalid', type = 'error', description = locale('component_invalid', label), position = 'top' })
	...
	lib.addKeybind({
		...
		onPressed = function(self)
			...
			if invBusy or not canOpenInventory() then
				return lib.notify({ id = 'inventory_player_access', type = 'error', description = locale('inventory_player_access'), position = 'top' })
			end
		...
		onPressed = function(self)
			...
			if currentWeapon.ammo then
				if currentWeapon.metadata.durability > 0 then
					...
					lib.notify({ id = 'no_durability', type = 'error', description = locale('no_durability', currentWeapon.label), position = 'top' })
				end
			end
		end
	})
...
RegisterNetEvent('ox_inventory:inventoryReturned', function(data)
	...
	lib.notify({ description = locale('items_returned'), position = 'top' })
	client.closeInventory()
...
RegisterNetEvent('ox_inventory:inventoryConfiscated', function(message)
	if source == '' then return end
	if message then lib.notify({ description = locale('items_confiscated'), position = 'top' }) end
	if currentWeapon then currentWeapon = Weapon.Disarm(currentWeapon) end
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
	...
	lib.notify({ description = locale('inventory_setup'), position = 'top' })
	Shops.refreshShops()
	Inventory.Stashes()
	Inventory.Evidence()
				...
				if currentInventory and not currentInventory.ignoreSecurityChecks then
					if currentInventory.type == 'otherplayer' then
						...
						if not id or #(playerCoords - pedCoords) > 1.8 or not (client.hasGroup(shared.police) or canOpenTarget(ped)) then
							client.closeInventory()
							lib.notify({ id = 'inventory_lost_access', type = 'error', description = locale('inventory_lost_access'), position = 'top' })
						else
...
RegisterNUICallback('removeComponent', function(data, cb)
	...
	if data.slot ~= currentWeapon.slot then
		return lib.notify({ id = 'weapon_hand_wrong', type = 'error', description = locale('weapon_hand_wrong'), position = 'top' })
	end
...
RegisterNUICallback('swapItems', function(data, cb)
	...
	elseif response then
		if type(response) == 'table' then
			...
			lib.notify({ type = 'error', description = locale(response), position = 'top' })
		end
	end
end)
...
RegisterNUICallback('craftItem', function(data, cb)
	...
	for i = 1, data.count do
		...
		if not success then
			if response then lib.notify({ type = 'error', description = locale(response or 'cannot_perform'), position = 'top' }) end
			break
		end
	end

(function(){
				...
				<div class="menu-items">
					{{#elements}}
						<div class="menu-item {{#selected}}selected{{/selected}}">
							<p class="item-label" {{#icon}}style="margin-top:2px;"{{/icon}}>
								{{#icon}}
									<i class="{{{icon}}}" style="margin-right:8px;opacity: 0.55;"></i> 
								{{/icon}}
								{{{label}}}
							</p>
							{{#isSlider}}
								<p class="item-slider">{{{sliderLabel}}}</p>							 
							{{/isSlider}}
						</div>
					{{/elements}}
				</div>
				<div class="hint">
					{{#elements}}
						{{#selected}}
							{{#hint}}
								<div class="menuHint">
									<p style="color:white"><i class="fas fa-info-circle"></i> {{{hint}}}</p>
								</div>
							{{/hint}}
							{{#hintImage}}
								<div class="menuHint">
	...
	window.ESX_MENU = {};
	ESX_MENU.ResourceName = 'crm-default';
	ESX_MENU.opened = {};
	ESX_MENU.focus = [];
	ESX_MENU.pos = {};
	...
	ESX_MENU.open = function(namespace, name, data) {
		...
		data._namespace = namespace; // resource name
		data._name = name; // title of menu

	...
	while true do
		...
		elseif WarMenu.IsMenuOpened('adminOpt') then
            ...
            if WarMenu.MenuButton('~HUD_COLOUR_TENNIS~Make an announcement', 'admin') then
				exports['crm-dialogA']:showDialog('adminmenu_announce', 'Make an Announcement:', '', 'This announcement will be showed in the chat to all online players.', onAnnouncementMade)
			end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('selfOptions') then
			...
			if WarMenu.Button('Change Ped Model') then
				exports['crm-dialogA']:showDialog('ped_change_menu', 'Enter Ped Model:', 'a_c_rat', 'List of Ped Models: https://wiki.rage.mp/index.php?title=Peds', onEnterPedModel)
			end
		...
		elseif WarMenu.IsMenuOpened('playerOptions') then
			...
			if WarMenu.MenuButton("Give Car", currentPlayer) then
				exports['crm-dialogA']:showDialog('adminmenu_givecar_'..currentPlayer, 'Car Model:', 'sultan', 'Specify the car model to spawn for '..svPlayers[currentPlayer].name..' - ID: '..currentPlayer, onGiveCarToPlayer)
			end
			...
			if WarMenu.MenuButton("~b~DM Player", "playerMan") then
				exports['crm-dialogA']:showDialog('adminmenu_text_'..currentPlayer, 'Your Messsage', '', 'Specify your message to '..svPlayers[currentPlayer].name..' - ID: '..currentPlayer, onTextPlayer)
			end
			if WarMenu.MenuButton("~y~Warn Player", "playerMan") then
				exports['crm-dialogA']:showDialog('adminmenu_warn_'..currentPlayer, 'Warn Reason', '', 'Specify the reason for warning '..svPlayers[currentPlayer].name..' - ID: '..currentPlayer, onWarnPlayer)
			end
			if WarMenu.MenuButton("~r~Kick Player", "playerMan") then
				exports['crm-dialogA']:showDialog('adminmenu_kick_'..currentPlayer, 'Kick Reason', '', 'Specify the reason for kicking '..svPlayers[currentPlayer].name..' - ID: '..currentPlayer..' | This action is irreversible!', onKickPlayer)
			end
			if WarMenu.MenuButton("~r~Ban Player", "playerMan") then
				exports['crm-dialogA']:showDialog('adminmenu_ban_'..currentPlayer, 'Ban Reason:', '', 'Specify the reason for banning '..svPlayers[currentPlayer].name..' - ID: '..currentPlayer..' | This action is irreversible!', onBanPlayer)
			end
			WarMenu.Display()
				...
				for k,v in pairs(Config.ESXAccounts) do
					if WarMenu.MenuButton("Set "..v.label, "manageAccs") then
						ESX.TriggerServerCallback("an_adminmenu:getAcc", function(amount)
							accType = v.name
							exports['crm-dialogA']:showDialog('set_player_acc_money', 'Set new '..v.label..' amount:', '0', 'Current '..v.label..' amount for '..svPlayers[currentPlayer].name..': $'..amount, onEnterNewAccAmount)
...
function onBanPlayer(text)
	...
	exports['crm-dialogA']:showDialog('adminmenu_ban2_'..currentPlayer, 'Ban Length:', '', 'Specify the ban duration (in days) for <b>'..svPlayers[currentPlayer].name..'</b> - ID: <b>'..currentPlayer..'</b> | This action is irreversible!', _onBanPlayer)			
end

...
local buttonHeight = 0.030
local buttonFont = 0
local buttonScale = 0.30
local buttonTextXOffset = 0.005
local buttonTextYOffset = 0.003
local buttonSpriteXOffset = 0.002
local buttonSpriteYOffset = 0.009
...
local defaultStyle = {
	...
	subTitleBackgroundColor = { 0, 0, 0, 210 },
	buttonPressedSound = { name = 'Click', set = 'DLC_HEIST_HACKING_SNAKE_SOUNDS' }, -- https://pastebin.com/0neZdsZ5
}
...
function WarMenu.Display()
	if currentMenu then
		...
		elseif IsControlJustReleased(0, keys.select) or IsDisabledControlJustReleased(0, keys.select) then

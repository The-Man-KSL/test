...
GetTimeDifference = function(time) {
    ...
    if(racetime < today) {
        $.post("https://crm-racing/leaverace", JSON.stringify({}));
    }
...
SetIncomingRace = function(incomingrace) {
    ...
    if (PlayerData) {
        if (incomingrace) {
            ...
            `) 
            //$.post("https://crm-racing/setinterval", JSON.stringify({timediff : incomingrace.data.Others.Time})); 
        }
    }
}
...
UpdateInterval = function(time) {
    ...
    $.post("https://crm-racing/updateinterval", JSON.stringify({timediff : diff}));
}
...
DeleteRoute = function(id) {
    $.post("https://crm-racing/DeleteRoute", JSON.stringify({id : id})); 
}
...
AddRace = function(data, route, id) {
    ...
    if (today > racetime) {
        //return $.post("https://crm-racing/clearracedata", JSON.stringify({id : id}));
    }
    $(".activelist").prepend(`
...
Select = function(type) {
    if (type == "exit") {
        $(".screen").fadeOut(300)
        $.post("https://crm-racing/CloseUi", JSON.stringify({}));
...
AttrMap = function(data, date, isroute,id){
    ...
    SetBG()
    $.post("https://crm-racing/ClearRouteData", JSON.stringify({})); 
}
...
$(document).on("dblclick", "#map", function(e) {
    ...
    TestFunct()
    $.post('https://crm-racing/AddMarker', JSON.stringify({x : latLng.lng, y : latLng.lat}));
});
    ...
    for (i = 0; i < RouteList.length; i++) {
        ...
        if (marker && !RouteList[i].eventcreated) {
            ...
            marker.on('dragend', function(event) {
                    ...
                    $.post('https://crm-racing/UpdateMarker', JSON.stringify({x : position.lng, y : position.lat, id : marker.options.id} ));
            });
        }
    }
}
...
$(document).on("click",".annountcement",function() {
    id = $(this).data("raceid")
    $.post('https://crm-racing/JoinRace', JSON.stringify({id : id}));
});
...
$(document).on("click",".joinrace",function() {
    ...
    $.post('https://crm-racing/JoinRace', JSON.stringify({id : raceid}));
});
...
$(document).on("click",".racejoin-way-join",function() {
    ...
    $.post('https://crm-racing/JoinRace', JSON.stringify({id : raceidt}));
});
...
CreateRace = function() {
    if (CheckList.Others.Time && CheckList.Others.Players && CheckList.Others.Route && CheckList.Name && CheckList.Reward) {
        $.post('https://crm-racing/CreateRace', JSON.stringify({data : CheckList, luadate : new Date(CheckList.Others.Time).getTime() }));
        $('#range').val(0);
...
CreateRoute = function() {
    ...
    if( isEmpty(val) ) {
        ...
        $.post('https://crm-racing/AddRoute', JSON.stringify({name : val}));
        RouteList = []
        //CheckList.Name = false
        //CheckList.Reward = false
        //CheckList.Others.Time = false

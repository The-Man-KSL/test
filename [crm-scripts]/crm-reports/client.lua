...
RegisterCommand(Config.FeedbackClientCommand, function(source, args, rawCommand)
	if canFeedback then
		...
		lib.notify({
			title = 'CRM REPORTS',
			description = 'You cannnot report so quickly.',
			position = 'top',
			type = 'error'
		})
	end
...
AddEventHandler('okokReports:NewFeedback', function(newFeedback)
	if staff then
		...
		lib.notify({
			title = 'CRM REPORTS',
			description = 'New report someone needs help!',
			position = 'top',
			type = 'success'
		})
		SendNUIMessage({
...
RegisterNUICallback("action", function(data)
	...
	if data.action == "newFeedback" then
		lib.notify({
			title = 'CRM REPORTS',
			description = 'Report sent to staff.',
			position = 'top',
			type = 'success'
		})
		...
		if FeedbackTable[data.feedbackid] then
			if oneSync then
				...
				if playerOnline then
					...
					lib.notify({
						title = 'CRM REPORTS',
						description = 'That player is not longer in the server.',
						position = 'top',
						type = 'error'
					})
				end

...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if inRange and not shown then
        	shown = true
        	exports['crm-textui']:Open('[E] To access the bank', 'darkblue', 'left') 
...
Citizen.CreateThread(function()
    ...
    while true do
            ...
            if IsControlJustReleased(0, 38) then
            	ESX.TriggerServerCallback("okokBanking:GetPIN", function(pin)
            		if pin then
						...
						lib.notify({
							title = 'CRM RP',
							description = 'Head up to a bank to setup a pin code',
							position = 'top',
							type = 'success'
						})
					end
        ...
        if inRange and not shown then
        	shown = true
        	exports['crm-textui']:Open('[E] To access the ATM', 'darkblue', 'left') 
        elseif not inRange and shown then
        	shown = false
        	exports['crm-textui']:Close()
        end
			...
			if tonumber(data.value) > 0 then
				ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
					...
					elseif isUsed == nil then
						lib.notify({
							title = 'CRM RP',
							description = 'This Iban does not exist',
							position = 'top',
							type = 'error'
						})
					end
		...
		if Config.CustomIBANAllowLetters then
			...
			ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
				...
				elseif isUsed ~= nil then
					lib.notify({
						title = 'CRM RP',
						description = 'This is iBan is invalid',
						position = 'top',
						type = 'error'
					})
				end
			...
			if tonumber(data.iban) ~= nil then
				...
				ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
					...
					elseif isUsed ~= nil then
						lib.notify({
							title = 'CRM RP',
							description = 'This iban is already in use',
							position = 'top',
							type = 'error'
						})
					end
				...
				lib.notify({
					title = 'CRM RP',
					description = 'You can only use numbers on your Iban',
					position = 'top',
					type = 'error'
				})
			end
	...
	elseif data.action == "change_pin" then
		if tonumber(data.pin) ~= nil then
			if string.len(data.pin) == 4 then
				...
				lib.notify({
					title = 'CRM RP',
					description = 'Your pin needs to be 4 digits',
					position = 'top',
					type = 'error'
				})
			end
			...
			lib.notify({
				title = 'CRM RP',
				description = 'You can only use numbers',
				position = 'top',
				type = 'error'
			})
		end
